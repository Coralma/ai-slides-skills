...
- **Logo watermark** at top-left of every slide (`max-height: clamp(28px, 4vh, 44px)`) — **MUST embed as base64 data URI** (do NOT use relative file paths — they break when HTML is generated in a different directory). Read the complete base64 string from `assets/logo-base64.txt` and use this exact img tag:
  ```html
  <img src="data:image/png;base64,{CONTENTS_OF_assets/logo-base64.txt}" alt="中宏保险" style="max-height: clamp(28px, 4vh, 44px); width: auto;" />
  ```
  The file `assets/logo-base64.txt` contains the full base64 encoding of the logo (PNG format, 374×428px, ~8KB). Always inline the complete data URI in generated HTML.
